...
const PALESTINE_API_URL = 'https://nexalo-api.vercel.app/api/palestine-v3';
...
module.exports.run = async function({ api, event, getText }) {
  ...
  try {
    ...
    if (stats.size === 0) throw new Error("Downloaded Palestine image is empty");

    // Construct the message
    const msg = {
      body: getText("palestine3", "success", targetName),
    ...
    console.error("[Palestine3 Command Error]", err.message);
    api.setMessageReaction("❌", messageID, () => {}, true);
    api.sendMessage(getText("palestine3", "error", err.message), threadID, messageID);
